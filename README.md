...
| Ratio | Owner | Role |
| :--- | :--- | :--- |
| **10%** | **Human** | **The Will (問いを立てる)**<br>Defining the core question and intent. |
| **80%** | **AI** | **The Acceleration (具体化)**<br>Execution, Coding, and Research at overwhelmed speed. |
| **10%** | **Human** | **The Decision (意思決定)**<br>Critical thinking and final judgment. |
...
graph TD
    ...
    classDef ai fill:#fff,stroke:#000,stroke-width:2px,color:#000;
    classDef db fill:#f9f9f9,stroke:#333,stroke-width:1px,stroke-dasharray: 5 5;

    %% Nodes
    H("Human / Will & Decision"):::human
    A("AI Agents / Context Engineering"):::ai
    P["Prototype / MVP"]
    M("Market / Validation"):::db
    DB[("Project Brain / NotebookLM")]:::db

    %% The Flow
    H -- "10%: Define Context" --> A
    A -- "80%: Vibe Coding & Execution" --> P
    P -- "Deploy" --> M
    M -- "Feedback & Raw Data" --> DB
    
    %% The Loop
    DB -. "Context & Trajectory" .-> A
    DB -. "Insight Extraction" .-> H
    H == "10%: Pivot Decision" ==> A
```
